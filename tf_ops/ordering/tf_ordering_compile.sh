...
/usr/local/cuda/bin/nvcc tf_ordering_g.cu -o tf_ordering_g.cu.o -c -O2 -DGOOGLE_CUDA=1 -x cu -Xcompiler -fPIC
...
# TF1.15 on L4T has ABI=1
  g++ -std=c++11 tf_ordering.cpp tf_ordering_g.cu.o -o tf_ordering_so.so -shared -fPIC -I $TF_INC -I /usr/local/cuda/include -I $TF_INC/external/nsync/public -lcudart -L /usr/local/cuda/lib64/ -L$TF_LIB -ltensorflow_framework -O2 -D_GLIBCXX_USE_CXX11_ABI=1
else
# TF1.15 on AMD64 has ABI=0
  g++ -std=c++11 tf_ordering.cpp tf_ordering_g.cu.o -o tf_ordering_so.so -shared -fPIC -I $TF_INC -I /usr/local/cuda/include -I $TF_INC/external/nsync/public -lcudart -L /usr/local/cuda/lib64/ -L$TF_LIB -ltensorflow_framework -O2 -D_GLIBCXX_USE_CXX11_ABI=0
fi
